...
// --- تعريف جميع الدوال في النطاق العام أولاً ---
// هذه الدوال يجب أن تكون متاحة لـ onclick في HTML
                ...
                <div id="variantsContainer">
                    ${isEdit && product.variants ? 
                        product.variants.map(v => `
                            <div class="variant-group" style="display: flex; gap: 10px; margin-bottom: 10px;">
                                <input type="text" class="variant-name" placeholder="اسم الخيار" value="${v.name}" required>
                                <input type="number" class="variant-stock" placeholder="المخزون" value="${v.stock}" min="0" required>
                                <button type="button" class="btn btn-danger" onclick="this.parentNode.remove()">حذف</button>
                            </div>
                        `).join('') : 
                        `<div class="variant-group" style="display: flex; gap: 10px; margin-bottom: 10px;">
                            <input type="text" class="variant-name" placeholder="اسم الخيار" required>
                            <input type="number" class="variant-stock" placeholder="المخزون" value="0" min="0" required>
                            <button type="button" class="btn btn-danger" onclick="this.parentNode.remove()">حذف</button>
                        </div>`
                    }
                </div>
...
window.showCouponForm = function(couponId = null) {
    const adminContent = document.getElementById('adminContent');
    adminContent.innerHTML = `
        <div class="admin-header">
            <h3>${couponId ? 'تعديل الكوبون' : 'إضافة كوبون جديد'}</h3>
            <button class="btn btn-secondary" onclick="loadAdminPanel()">العودة لإدارة الكوبونات</button>
        </div>
        <p style="color: #666; margin-bottom: 1rem;">${couponId ? 'تعديل الكوبون' : 'إضافة كوبون خصم جديد'}</p>
...
window.loadAdminPanel = async function() {
    ...
    adminContent.innerHTML = `
        ...
        <div id="couponsTab" class="admin-tab-content">
            ...
            <div id="couponsList">
                <p style="color: #666; text-align: center; padding: 2rem;">هذه الميزة قيد التطوير</p>
            </div>
        </div>
    `;
...
// --- الدوال الداخلية (غير معروضة في النطاق العام) ---
...
async function updateCartTotal() {
    ...
    document.getElementById('finalCheckoutTotal').textContent = total.toFixed(2) + ' ريال';
}
...
async function handleCheckout(e) {
    ...
    try {
        ...
        const whatsappPhone = '249933002015';
...
async function loadAdminProducts() {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            html += `
                <tr>
                    <td><img src="${product.image || 'https://via.placeholder.com/50x50'}" alt="${product.nameAr}" style="width: 50px; height: 50px; object-fit: cover;"></td>
